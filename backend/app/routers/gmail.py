...
async def sync_emails(session: AsyncSession = Depends(get_session)):
    ...
    if not new_msg_ids:
        return {"status": "up_to_date", "synced": 0, "total": len(messages)}
    ...
    if not email_details:
        return {"status": "fetch_failed", "synced": 0}
    ...
    logger.info("Synced %d new emails from Gmail.", stored_count)
...
async def get_all_emails(
    ...
    """Return all emails with optional category filter."""
    stmt = select(EmailNotification).order_by(EmailNotification.received_at.desc()).limit(limit)
    if category:
