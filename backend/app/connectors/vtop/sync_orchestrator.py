...
class SyncOrchestrator:
    ...
    async def sync_all(self, semester_id: str) -> SyncResult:
        ...
        try:
            ...
            # Scrape timetable
            timetable_records = await self._scrape_timetable(semester_id)
            if timetable_records is None:
                return SyncResult(
                    status="session_expired",
                    error="Session expired while scraping timetable.",
                )
            ...
            return SyncResult(
                ...
                timetable_count=len(timetable_records),
    ...
    async def _scrape_timetable(self, semester_id: str) -> list[dict] | None:
        """Scrape timetable data for a semester via StudentTimeTableChn.
        ...
        logger.info("Scraped %d timetable slots.", len(records))
        return records
